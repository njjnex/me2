...
<c:forEach var="userJ" items="${event.eventMembersJoined}">
	...
	<c:choose>
        ...
        <h3><a class ="event-name" href="${pageContext.request.contextPath}/events/${event.id}"> ${event.eventName}</a>
        	<span>
        		<img src="${pageContext.request.contextPath}/static/event${event.id}.jpg" onerror="if (this.src != 'event${event.id}.jpg') this.src = '${pageContext.request.contextPath}/resources/images/events/template.png';"
									...
									<c:otherwise>
										<c:choose>
											<c:when test="${event.createdBy.username ne activeUser}">
												<button type="button" class="btn btn-success" onclick="window.location.href='${pageContext.request.contextPath}/${event.id}/joinEvent.html'">Участвую!</button>
												<% alreadyJoined = true; %>
											</c:when>
											<c:otherwise>
												<c:choose>
